        ...
        {users.map((user) => (
          <div key={user._id} className="p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-gray-600 text-sm truncate max-w-[150px]">{user.email}</p>
            </div>
        ...
        <div className="divide-y divide-gray-100">
          {users.map((user) => (
            <div
              ...
              {/* Truncated ID */}
              <div className="text-gray-700 text-sm truncate max-w-[100px]">{user._id.slice(0, 10)}...</div>

              {/* Name */}
              <div className="text-gray-900 text-base font-medium">{user.name}</div>

              {/* Email */}
              <div className="text-gray-700 text-base truncate max-w-[250px]">{user.email}</div>
              {/* DOB */}
              <div className="text-gray-700 text-sm">{user.dateOfBirth ? new Date(user.dateOfBirth).toISOString().slice(0, 10) : ""}</div>
              {/* Actions */}
